![difftastic for Debian](.github/readme-header.png)

# difftastic for Debian

[Wilfred/difftastic](https://github.com/Wilfred/difftastic) — A structural diff tool that compares files based on syntax rather than lines, understanding 30+ programming languages. —
...
Or download a `.deb` from the [Releases](https://github.com/latest-debs/difftastic-debian/releases) page:

```sh
sudo dpkg -i difftastic_*.deb
```
...
- amd64, arm64, armhf, i386, ppc64el, riscv64, s390x — whichever
  architectures Wilfred/difftastic actually publishes a Linux binary for
...
For issues with difftastic itself, see
